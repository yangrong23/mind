...
import { isNoteProcessing, isNoteRecording } from "@/lib/note-status"
...
function NotesRecordGlowIcon({
  ...
  return (
    ...
    >
      ...
      <Mic className={cn("relative z-[1]", iconClassName, "text-white dark:text-[#1a1a1a]")} strokeWidth={2.1} aria-hidden />
    </div>
  )
}

const notesEntryBtn =
  "flex min-h-[40px] flex-1 items-center justify-center gap-1.5 rounded-xl border border-stone-200/90 bg-stone-50/60 text-zinc-700 transition-colors hover:bg-stone-100/80 active:bg-stone-100 dark:border-zinc-700/90 dark:bg-zinc-900/50 dark:text-zinc-200 dark:hover:bg-zinc-800/80"

/** Record + new note — peer actions, equal weight, compact. */
function NotesDualEntryBar({
  ...
  return (
    <div className="shrink-0 border-t border-stone-100/90 bg-white/95 px-3 py-2 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-950/95">
      <div className="flex gap-2">
        <button type="button" onClick={onRecord} className={notesEntryBtn} aria-label="Start recording">
          <Mic className="h-4 w-4 shrink-0 text-zinc-500 dark:text-zinc-400" strokeWidth={1.85} aria-hidden />
          <span className="text-[13px] font-medium">Record</span>
        </button>
        <button type="button" onClick={onNewNote} className={notesEntryBtn} aria-label="New note">
          <FileText className="h-4 w-4 shrink-0 text-zinc-500 dark:text-zinc-400" strokeWidth={1.85} aria-hidden />
...
export function NotesTab({
  ...
  return (
    <div className={cn("relative flex h-full flex-col", "bg-[#fafaf9] dark:bg-zinc-950")}>
      <div className={cn("border-b bg-white/95 backdrop-blur-sm", "border-[#e5e3df] dark:border-zinc-800")}>
          ...
          <div className="flex min-w-0 items-center gap-0.5">
            ...
            >
              <RecorderGlyph
                ...
                className={cn(
                  isDeviceConnected ? "text-white dark:text-[#1a1a1a]" : "text-zinc-500 dark:text-zinc-400"
                )}
              />
            </button>
            <button
      ...
      <div className="flex-1 overflow-y-auto overscroll-y-contain px-0 pb-2 pt-0 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <div className="divide-y divide-zinc-100/90 px-4 dark:divide-zinc-800/80">
            {filteredNotes.map((note) =>
              isNoteProcessing(note) ? (
                ...
                <RecordingMemoRow key={note.id} note={note} onOpen={() => openNote(note)} />
              ) : (
                <SwipeableMemoCard
                  key={note.id}
                  note={note}
      ...
      {fileScope !== "trash" && textEditor === null ? (
        <NotesDualEntryBar
          onRecord={() => setShowRecordOptions(true)}
          onNewNote={openNewRichTextNote}
        />
      ) : null}
      ...
      {showRecordOptions && (
        <div className="absolute inset-0 z-40">
          ...
          />
          <div className="absolute bottom-0 left-0 right-0 animate-in slide-in-from-bottom rounded-t-3xl bg-white duration-300 dark:bg-zinc-950">
            <div className="flex justify-center pb-2 pt-3">
              <div className="h-1 w-10 rounded-full bg-stone-200/90 dark:bg-zinc-700" />
            </div>
            <div className="px-5 pb-2">
              <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Record</h3>
              <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Start a capture or open device details</p>
            </div>
            <div className="space-y-2 px-5 pb-6">
              <button
                ...
                className={cn(
                  "flex w-full items-center gap-3 rounded-xl border border-stone-200/85 py-3.5 pl-4 pr-3 text-left dark:border-zinc-700",
              ...
              <button
                ...
                className="flex w-full items-center gap-3 rounded-xl border border-stone-200/85 bg-white py-3.5 pl-4 pr-3 text-left hover:bg-stone-50 dark:border-zinc-700 dark:bg-zinc-900 dark:hover:bg-zinc-800"
              >
                <div
                  className={cn(
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[15px] font-semibold text-zinc-900 dark:text-zinc-100">Source & devices</div>
                  <div className="text-[12px] text-zinc-500 dark:text-zinc-400">Battery, storage, firmware, pairing</div>
                </div>
                ...
                <div className="space-y-0.5 border-b border-stone-100/80 pb-4">
                  {(
                    ...
                    <button
                      ...
                      className={cn(
                        "flex w-full items-center gap-3 rounded-xl py-3.5 pl-1 pr-2 text-left transition-[background-color,box-shadow]",
                        row.active
                          ? cn(
                              "bg-white font-medium text-zinc-900 dark:bg-zinc-900 dark:text-zinc-100",
                              web.navSelectionShadow
                            )
