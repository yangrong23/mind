            ...
            {sessions.map((s) => (
              <button
                ...
                className={cn(
                  ...
                  activeSession === s.id
                    ? "bg-gradient-to-br from-teal-50/95 to-sky-50/80 font-medium text-zinc-800"
                    : "hover:bg-stone-50"
                )}
              >
                ...
                onAtClick={() =>
                  runWithAuth(() =>
                    toast.message("Mention", {
                      description: libraryAssistant
                        ? `Ground on ${scopedLibraryName ?? "this library"} (demo).`
                        : "Pick a library or source (demo).",
                    })
                  )
                }
                atTitle={libraryAssistant?.displayName ?? "Libraries & sources"}
              />
            </div>
          </div>
        </section>
