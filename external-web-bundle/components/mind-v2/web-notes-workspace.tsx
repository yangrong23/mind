...
export function WebNotesWorkspace({
  ...
  return (
    <div className={cn("flex h-full min-h-0", web.canvas, webNavMotion.contentEnter)}>
      ...
      <aside
        className={cn(
          "flex h-full shrink-0 flex-col border-r border-stone-200/70 bg-[#f5f5f6]/90",
          web.secondaryWidth
        )}
        aria-label="Notes list"
      >
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          <h1 className="text-[18px] font-semibold tracking-tight text-zinc-700">All notes</h1>
          <div className="flex items-center gap-0.5">
            <button
              type="button"
              className={cn("rounded-lg p-2 text-zinc-500 hover:bg-white/80", webNavMotion.pressable)}
        ...
        <div className="px-3 pb-2">
          <input
            ...
            className="w-full rounded-xl border-0 bg-white px-3 py-2 text-[13px] text-zinc-700 shadow-[0_1px_3px_rgba(0,0,0,0.05)] ring-1 ring-black/[0.04] placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-teal-200/60"
          />
        </div>
            ...
            {!isTextNote ? (
              <div className="rounded-2xl border border-dashed border-stone-200 bg-stone-50/80 px-6 py-10 text-center">
                ...
                <p className="mt-2 text-[13px] leading-relaxed text-zinc-500">
                  This capture opens on mobile for playback and transcript. Create a text note to edit on web.
                </p>
              </div>
            ) : (
              <>
        ...
        >
          ...
          <div className="min-h-0 flex-1">
            <AgentChat
              ...
              entryHint={`Co-write “${title.trim() || selected?.title || "this note"}”—grounded on your note.`}
              noteContext={{
