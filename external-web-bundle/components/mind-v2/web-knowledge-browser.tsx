...
const SECTIONS: {
  id: "mine" | "team" | "subscribed"
  label: string
}[] = [
  { id: "mine", label: "Personal" },
  { id: "team", label: "Shared" },
...
export function WebKnowledgeBrowser({
  ...
  const [subscribedGroupOpen, setSubscribedGroupOpen] = useState<Record<SubscribedKbRole, boolean>>({
    published: true,
    followed: true,
  })
  ...
  const grouped = useMemo(() => {
    const base = {
      ...
      subscribed: allKBs.filter((k) => k.category === "subscribed"),
    }
    ...
    return Object.fromEntries(
      SECTIONS.map((s) => [s.id, sortKnowledgeBases(filtered[s.id], kbSort)])
    ) as typeof base
  }, [allKBs, kbQuery, kbSort])

  useEffect(() => {
    if (!kbQuery) return
    setExpanded({ mine: true, team: true, subscribed: true })
    setSubscribedGroupOpen({ published: true, followed: true })
  }, [kbQuery])

  const subscribedByRole = useMemo(() => {
    const items = grouped.subscribed
    return {
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-2 pt-3 pb-0">
          {SECTIONS.map((section) => {
            ...
            if (section.id === "subscribed") {
              const subscribedEmpty =
                subscribedByRole.published.length === 0 && subscribedByRole.followed.length === 0
              return (
                <div key={section.id}>
                  <button
                    type="button"
                    onClick={() => setExpanded((e) => ({ ...e, subscribed: !e.subscribed }))}
                      ...
                      <button
                        ...
                        onClick={(ev) => {
                          ...
                          runAuth(() => {
                            ...
                            else toast.message("Library plaza", { description: "Discover libraries to subscribe." })
                          })
                        }}
                    ...
                    <div className="mt-0.5 space-y-2 pl-0.5">
                      {subscribedEmpty ? (
                        ...
                        SUBSCRIBED_GROUPS.map((group) => {
                          ...
                          const groupOpen = subscribedGroupOpen[group.id]
                          ...
                          return (
                            <div key={group.id}>
                              <button
                                type="button"
                                onClick={() =>
                                  setSubscribedGroupOpen((s) => ({ ...s, [group.id]: !s[group.id] }))
                                }
                                className="flex w-full items-center justify-between rounded-md px-2 py-1 text-left hover:bg-white/50"
                              >
                                <span className="text-[12px] font-medium text-zinc-500">{group.label}</span>
                                <ChevronRight
                                  className={cn(
                                      ...
                                      <li key={kb.id}>
                                        <button
                                          ...
                                          className={webNavListItem(active, {
                                            className:
                                              "flex w-full items-center gap-2.5 px-2 py-2 text-left text-[13px] font-medium",
                                          })}
                                        >
                    ...
                    <button
                      ...
                      onClick={(ev) => {
                        ...
                        runAuth(() =>
                          setCreateDialogMode({
                            kind: "create",
                            category: section.id as "mine" | "team",
                          })
                        )
                      }}
                      className="rounded-md p-0 text-zinc-400 hover:bg-stone-100 hover:text-zinc-600"
