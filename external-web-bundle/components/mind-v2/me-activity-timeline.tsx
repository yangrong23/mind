  ...
  return (
    ...
    >
      <div className="animate-[me-timeline-bubble-float_3s_ease-in-out_infinite]">
        ...
        >
        <span
          className="pointer-events-none absolute -left-3 -top-3 h-10 w-10 rounded-full bg-sky-300/25 blur-md"
          aria-hidden
        />
        <span
          className="pointer-events-none absolute -right-2 bottom-0 h-8 w-8 rounded-full bg-teal-300/20 blur-md"
          aria-hidden
        />
        <span
          className="absolute inset-x-3 top-0 h-px bg-gradient-to-r from-transparent via-sky-300/80 to-transparent"
          aria-hidden
        />
        <p className="relative text-center text-[10px] font-semibold tracking-[0.06em] text-sky-600/90 dark:text-sky-300/90">
          {day.homeDateLabel}
        </p>
        <p className="relative mt-0.5 line-clamp-2 text-center text-[11px] font-semibold leading-[1.35] text-zinc-800 dark:text-zinc-50">
          {day.title}
        </p>
        </div>
      </div>
      <span
        ...
        aria-hidden
      >
        <span className="absolute h-2.5 w-2.5 rotate-45 rounded-[2px] border border-sky-200/80 bg-white/95 shadow-sm dark:border-sky-500/30 dark:bg-zinc-900/95" />
...
export function MeTimelineHeatmapGrid({
  ...
  const cellPx = size === "lg" ? "min-h-[32px]" : "min-h-[26px]"

  return (
    <div
      className={cn(
        "relative overflow-visible rounded-xl border border-stone-200/90 bg-gradient-to-b from-white to-stone-50/80 dark:border-zinc-700/90 dark:from-zinc-900 dark:to-zinc-950/80",
        sequentialTitles ? "px-3 pb-3 pt-2" : "p-3",
        className
      )}
    >
      <div
        className="pointer-events-none absolute -right-6 top-0 h-24 w-24 rounded-full bg-mind/10 blur-2xl"
        aria-hidden
      />
      <div
        className="relative grid gap-[5px] overflow-visible"
      ...
      >
        {gridDays.map((day, i) => {
          ...
          const glow =
            isActive ? (
              <span
                className="pointer-events-none absolute inset-0 rounded-[5px] ring-1 ring-mind/30 animate-[me-timeline-cell-glow_2.8s_ease-in-out_infinite]"
                aria-hidden
              />
            ) : null
          ...
          const cellBody = (
            <div className="relative aspect-square w-full">
              ...
              {isActive ? <TimelineTitleBubble day={day} activeIdx={activeSlot} /> : null}
            </div>
          )
          ...
          if (onSelectDay) {
            return (
              <button
                ...
                className="relative w-full rounded-[5px] focus:outline-none focus-visible:ring-2 focus-visible:ring-mind/50"
...
function TimelineFlowNode({
  ...
  return (
    ...
    >
      <span
        className={cn(
          "absolute left-1/2 top-1/2 block -translate-x-1/2 -translate-y-1/2 rounded-full bg-sky-400/20",
          prominent ? "h-[22px] w-[22px]" : "h-[18px] w-[18px]"
        )}
      />
      <span
        className={cn(
          "absolute left-1/2 top-1/2 block -translate-x-1/2 -translate-y-1/2 rounded-full bg-mind shadow-[0_0_10px_rgba(56,189,248,0.45)] ring-[2.5px] ring-white dark:ring-zinc-900",
        ...
        >
          <defs>
            <linearGradient id={`${flowUid}-grad`} x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stopColor="rgb(56 189 248 / 0.55)" />
              <stop offset="55%" stopColor="rgb(56 189 248 / 0.85)" />
              <stop offset="100%" stopColor="rgb(45 212 191 / 0.45)" />
            </linearGradient>
          </defs>
          {days.length > 1 ? (
            <>
            ...
            if (onSelectDay) {
              return (
                <button
                  ...
                  type="button"
                  onClick={() => onSelectDay(day)}
                  className="relative block w-full rounded-xl text-left transition-colors hover:bg-teal-50/40 active:bg-teal-50/60 dark:hover:bg-teal-950/30"
...
export function MeCaptureDiaryWebCard({
  ...
  return (
    <section
      className={cn(
        "rounded-2xl border border-white/90 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900",
        className
      )}
    >
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={onOpenDiary}
          className="shrink-0 rounded-lg px-2.5 py-1.5 text-[12px] font-semibold text-mind hover:bg-teal-50/80"
        >
          See all
        </button>
      </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      activityHeatmapCellClass(day.activity),
                      selected
                        ? "ring-2 ring-mind ring-offset-1 ring-offset-stone-50 dark:ring-offset-zinc-950"
                        : "hover:opacity-90"
                    )}
        ...
        <div className={cn("flex min-h-0 flex-1", webLayout && "gap-0 p-2")}>
          <div
            className={cn(
              ...
              webLayout && "rounded-2xl bg-white/90 ring-1 ring-black/[0.04]",
              webLayout && web.panelShadow
            )}
          >
