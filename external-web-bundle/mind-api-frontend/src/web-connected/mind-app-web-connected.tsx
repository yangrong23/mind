...
import { MeTab } from "@/components/mind-v2/me-tab"
      ...
      <div className={cn("flex h-screen min-h-0 w-full overflow-hidden", web.canvas)}>
        ...
        {shellMain && !settingsOpen && activeTab === "agent" ? (
          <WebAgentSidebar
            ...
            onNewChat={() => {
              ...
              toast.message("New chat", { description: "Ready for a fresh thread." })
            }}
            selectedHistory={selectedHistory}
            onSelectHistory={(title) => {
              setSelectedHistory(title)
              setAgentHistoryDraft(title)
            }}
          />
        ) : null}
        ...
        >
          ...
          {shellMain && !settingsOpen && activeTab === "plaza" && (
            <WebPlazaDiscoverPage onPickLibrary={(kb) => openNotebook(kb)} />
          )}

          {shellMain && !settingsOpen && activeTab === "notes" && (
