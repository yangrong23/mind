                  ...
                  <div v-if="formData" class="section-content">
                    ...
                    <div class="section-body">
                      ...
                      <div v-if="!isFAQ && formData.indexingStrategy.wikiEnabled" class="form-item">
                        ...
                        <t-input-number
                          v-model="formData.wikiConfig.maxPagesPerIngest"
                          :min="0"
                          :max="500"
                          style="width: 200px;"
                        />
...
import { createKnowledgeBase, getKnowledgeBaseById, listKnowledgeFiles, updateKnowledgeBase, rebuildKBIndex, getKBCreationTemplate } from '@/api/knowledge-base'
...
const validateForm = (): boolean => {
  ...
  // 模型字段留空意味着「跟随系统默认」：后端 autoFillBuiltinModels 会自动填充
  // (Embedding 锁定为内置 UUID；Summary/VLM/ASR 使用哨兵值在调用时解析)。
  // 仅在编辑模式下，对已存在文件的 KB，强制要求 Embedding 不能被清空。
  if (props.mode === 'edit' && hasFiles.value) {
  ...
  // 多模态如果用户显式开启，则需要选模型；未开启时无需校验（后端用哨兵值）
...
const buildSubmitData = () => {
  ...
  // 创建模式下，空的模型 ID 不传，让后端 autoFillBuiltinModels 用哨兵值/内置模型填充。
  // 编辑模式下保留旧行为（编辑接口会区分未传 vs 显式清空，由后端 PATCH 语义处理）。
  if (isCreate) {
  ...
  // 添加多模态配置
  // 创建模式：用户保持默认关时不传 vlm_config，让后端从 template 拷贝（默认开+sentinel）
  // 编辑模式或用户改过开关：传完整配置，关闭时带 sentinel 表达"明确关闭"
  if (isCreate && !formData.value.multimodalConfig.enabled && !formData.value.multimodalConfig.vllmModelId) {
    // 用户未动开关，不传字段让后端用模板默认
  } else {
    data.vlm_config = {
      ...
      model_id: formData.value.multimodalConfig.enabled
        ? (formData.value.multimodalConfig.vllmModelId || '')
        : '__builtin__'  // 关闭时带 sentinel，防止后端 zero-value 检查覆盖
    }
  }

  // 添加ASR语音识别配置（同 vlm_config 逻辑）
  if (isCreate && !formData.value.asrConfig?.enabled && !formData.value.asrConfig?.modelId) {
    // 用户未动开关，不传字段让后端用模板默认
  } else {
    data.asr_config = {
      ...
      model_id: formData.value.asrConfig?.enabled
        ...
        : '__builtin__',  // 关闭时带 sentinel
      language: formData.value.asrConfig?.language || ''
    }
  }

  // 存储引擎：仅传 provider，参数从全局设置读取
  // 创建模式下，storageProvider 留空时让后端 autoFillBuiltinModels 用租户配置/STORAGE_TYPE/minio 兜底
  if (isCreate) {
    if (formData.value.storageProvider) {
      data.storage_provider_config = { provider: formData.value.storageProvider }
      data.storage_config = { provider: formData.value.storageProvider }
    }
  } else {
  ...
  if (formData.value.type !== 'faq') {
    ...
    // 创建模式：max_pages_per_ingest 为 0 时不传，让后端从 template 拷贝（默认 50）
    // 编辑模式：保留用户设置的值（包括 0）
    if (!isCreate || (formData.value.wikiConfig?.maxPagesPerIngest && formData.value.wikiConfig.maxPagesPerIngest > 0)) {
      data.wiki_config.max_pages_per_ingest = formData.value.wikiConfig?.maxPagesPerIngest || 0
    }
  }
...
watch(() => props.visible, async (newVal) => {
  if (newVal) {
    ...
    if (props.mode === 'edit' && props.kbId) {
      ...
      // 创建模式：先初始化空表单，再异步拉后端默认存储引擎填充，避免 UI 闪现 'local'
      formData.value = initFormData(props.initialType || 'document')
      hasFiles.value = false
      // 并行拉取存储引擎配置和 KB 创建模板，用实际系统默认值初始化表单
      const [storageResp, templateResp] = await Promise.allSettled([
        getStorageEngineConfig(),
        getKBCreationTemplate(),
      ])
      if (storageResp.status === 'fulfilled' && formData.value) {
        const provider = (storageResp.value as any)?.data?.default_provider
        if (provider && !formData.value.storageProvider) {
          formData.value.storageProvider = provider
        }
      }
      if (templateResp.status === 'fulfilled' && formData.value) {
        const tpl = (templateResp.value as any)?.data
        if (tpl) {
          // 用 template 的实际默认值覆盖硬编码的 false，让用户看到真实的系统默认配置
          formData.value.multimodalConfig.enabled = !!tpl.vlm_config?.enabled
          // When template has VLM enabled with a builtin/sentinel model, carry it over
          // so the frontend validator (enabled && !vllmModelId) doesn't block submission.
          if (tpl.vlm_config?.enabled && !formData.value.multimodalConfig.vllmModelId) {
            formData.value.multimodalConfig.vllmModelId = tpl.vlm_config?.model_id || '__builtin__'
          }
          formData.value.asrConfig.enabled = !!tpl.asr_config?.enabled
          if (tpl.asr_config?.enabled && !formData.value.asrConfig.modelId) {
            formData.value.asrConfig.modelId = tpl.asr_config?.model_id || '__builtin__'
          }
          formData.value.nodeExtractConfig.enabled = !!tpl.extract_config?.enabled
          if (tpl.indexing_strategy) {
            formData.value.indexingStrategy.wikiEnabled = !!tpl.indexing_strategy.wiki_enabled
            formData.value.indexingStrategy.graphEnabled = !!tpl.indexing_strategy.graph_enabled
          }
        }
      }
    }
