...
import ArtifactListView from './components/ArtifactListView.vue';
...
const validTabs = ['documents', 'wiki', 'graph', 'artifacts'] as const
...
// ── 工作区模式（Workspace）──────────────────────────────────────────
// 当 URL 带 ?view=workspace 时进入 3 面板 Workspace 布局
const isWorkspaceMode = computed(() => route.query.view === 'workspace')
// Workspace 内的活跃面板 tab（studio 内部 tabs）
type WorkspaceStudioTab = 'artifacts' | 'graph'
const activeWsStudioTab = ref<WorkspaceStudioTab>('artifacts')

// 退出工作区：移除 view=workspace 查询参数
const exitWorkspace = () => {
...
<template>
  <template v-if="!isFAQ">
    ...
    <!-- ===== Workspace 三面板布局 ===== -->
    <div v-if="isWorkspaceMode" class="ws-canvas">
      <!-- 顶部 Header -->
      <div class="ws-header">
        <div class="ws-header-left">
          <button type="button" class="ws-back-btn" @click="exitWorkspace">
            <t-icon name="chevron-left" size="16px" />
            <span>{{ $t('menu.knowledgeBase') }}</span>
          </button>
          <span class="ws-header-sep">/</span>
          <span class="ws-header-title">{{ kbInfo?.name || '' }}</span>
        </div>
        <div class="ws-header-right">
          <t-tooltip :content="$t('knowledgeBase.settings')" placement="bottom">
            <button v-if="canManage" type="button" class="ws-icon-btn" @click="handleOpenKBSettings">
      ...
      <div class="ws-body">
        <!-- Sources 面板 -->
        <aside class="ws-panel ws-panel--sources" aria-label="Sources">
          <div class="ws-panel-header">
            <span class="ws-panel-title">
              <t-icon name="file-copy" size="15px" />
              Sources
            </span>
            <div v-if="canEdit" class="ws-panel-actions">
              <t-dropdown
          ...
          <div class="ws-panel-body">
            ...
            <input ref="folderUploadInputRef" type="file" class="document-upload-input" webkitdirectory @change="handleFolderUpload" />
            <!-- 文档列表 -->
            <div class="ws-sources-search">
              <t-input v-model.trim="docSearchKeyword" :placeholder="$t('knowledgeBase.docSearchPlaceholder')" clearable size="small"
                @clear="loadKnowledgeFiles(kbId)" @keydown.enter="loadKnowledgeFiles(kbId)">
                <template #prefix-icon><t-icon name="search" size="14px" /></template>
              </t-input>
            </div>
            <div class="ws-sources-list" ref="knowledgeScroll" @scroll="handleScroll">
              <div v-if="docListLoading && cardList.length === 0" class="ws-sources-skeleton">
                <div v-for="n in 6" :key="'ws-skel-'+n" class="ws-source-item ws-source-item--skeleton">
                  <t-skeleton animation="gradient" :row-col="[{ width: '70%', height: '14px' }, { width: '50%', height: '11px' }]" />
                </div>
              </div>
              <template v-else-if="cardList.length">
                ...
                >
                  <div class="ws-source-icon">
                    <t-icon :name="item.type === 'url' ? 'link' : 'file'" size="14px" />
                  </div>
                  <div class="ws-source-info">
                    <span class="ws-source-name" :title="item.file_name">{{ item.file_name }}</span>
                    <span class="ws-source-meta">{{ formatFileSize(item.size || 0) }}</span>
                  </div>
                  <div v-if="canEdit" class="ws-source-more" @click.stop>
                    <t-popup overlayClassName="card-more" trigger="click" destroy-on-close placement="bottom-right">
                      <button type="button" class="ws-icon-btn ws-icon-btn--sm">
                        <t-icon name="ellipsis" size="13px" />
                      </button>
                      <template #content>
                        <div class="card-menu">
                          <div class="card-menu-item" @click.stop="handleKnowledgeReparse(cardList.indexOf(item), item)">
                            <t-icon class="icon" name="refresh" /><span>{{ t('knowledgeBase.rebuildDocument') }}</span>
                          </div>
                          <div class="card-menu-item danger" @click.stop="delCard(cardList.indexOf(item), item)">
                            <t-icon class="icon" name="delete" /><span>{{ t('knowledgeBase.deleteDocument') }}</span>
                          </div>
                        </div>
                      </template>
                    </t-popup>
              ...
              <div v-else-if="!docListLoading" class="ws-sources-empty">
                <t-icon name="file-copy" size="32px" />
                <p>{{ $t('knowledgeBase.docEmptyResult') }}</p>
              </div>
            </div>
          </div>
        </aside>
        ...
        <!-- Dialogue 面板（摘要 + 提问入口） -->
        <section class="ws-panel ws-panel--dialogue" aria-label="Dialogue">
          <div class="ws-panel-header">
            <span class="ws-panel-title">
              <t-icon name="chat" size="15px" />
              Dialogue
            </span>
          </div>
          <div class="ws-panel-body ws-dialogue-body">
            <!-- KB 摘要卡片 -->
            <div class="ws-summary-card">
              <div class="ws-summary-icon">
                <t-icon name="book-open" size="20px" />
              </div>
              <div class="ws-summary-content">
                <h3 class="ws-summary-title">{{ kbInfo?.name || '' }}</h3>
                <p class="ws-summary-desc">{{ kbInfo?.description || $t('knowledgeBase.noDescription') }}</p>
                <div class="ws-summary-stats">
                  <span><t-icon name="file-copy" size="12px" /> {{ cardList.length }} {{ $t('knowledgeList.itemsLabel') }}</span>
                </div>
              </div>
            </div>

            <!-- 快速提问入口 -->
            <div class="ws-ask-area">
              <div class="ws-ask-card" @click="createNewSession">
                <div class="ws-ask-icon">
                  <t-icon name="chat-bubble" size="18px" />
                </div>
                <div class="ws-ask-text">
                  <span class="ws-ask-title">{{ $t('knowledgeBase.startChat') || '开始对话' }}</span>
                  <span class="ws-ask-subtitle">{{ $t('knowledgeBase.askAboutContent') || '向知识库提问，获取智能摘要' }}</span>
                </div>
                <t-icon name="chevron-right" size="16px" class="ws-ask-arrow" />
              </div>
            </div>

            <!-- Wiki 区域（如果启用） -->
            <div v-if="isWiki" class="ws-wiki-section">
              <div class="ws-section-label">Wiki</div>
              <div class="ws-wiki-entry" @click="activeWsStudioTab = 'wiki'; exitWorkspace(); activeKbTab = 'wiki'">
                <t-icon name="book" size="15px" />
                <span>{{ $t('knowledgeEditor.wikiBrowser.tabDocuments') }}</span>
                <t-icon name="chevron-right" size="14px" style="margin-left: auto" />
              </div>
            </div>
          </div>
        </section>

        <!-- Studio 面板（Artifacts + Knowledge Graph） -->
        <aside class="ws-panel ws-panel--studio" aria-label="Studio">
          <div class="ws-panel-header">
            <span class="ws-panel-title">
              <t-icon name="layers" size="15px" />
              Studio
            </span>
            <div class="ws-studio-tabs">
              <button
                class="ws-studio-tab"
                :class="{ active: activeWsStudioTab === 'artifacts' }"
                @click="activeWsStudioTab = 'artifacts'"
              >Artifacts</button>
              <button
                v-if="isWiki"
                class="ws-studio-tab"
                :class="{ active: activeWsStudioTab === 'graph' }"
                @click="activeWsStudioTab = 'graph'"
              >{{ $t('knowledgeEditor.wikiBrowser.tabGraph') }}</button>
            </div>
          </div>
          <div class="ws-panel-body ws-studio-body">
            <ArtifactListView v-if="activeWsStudioTab === 'artifacts' && kbId" :knowledge-base-id="kbId" />
            <WikiBrowser v-else-if="activeWsStudioTab === 'graph' && kbId && isWiki"
              :knowledge-base-id="kbId" view="graph"
    ...
    <div v-else class="knowledge-layout">
          ...
          <div class="document-title-row">
            <h2 class="document-breadcrumb">
              ...
              <template v-if="isWiki">
                ...
                <span class="breadcrumb-tab-sep">/</span>
                <span
                  :class="['breadcrumb-tab', { active: activeKbTab === 'artifacts' }]"
                  @click="activeKbTab = 'artifacts'"
                >
                  Artifacts
                </span>
              </template>
      ...
      <!-- Artifacts tab -->
      <ArtifactListView v-if="activeKbTab === 'artifacts' && kbId" :knowledge-base-id="kbId" />

      <template v-if="(activeKbTab === 'documents' || !isWiki) && activeKbTab !== 'artifacts'">
      <input
        ref="uploadInputRef"
        type="file"
...
.ws-canvas {
  ...
  width: 100%;
  min-width: 0;
  background: var(--td-bg-color-page, #eee);
  overflow: hidden;
}
...
.ws-panel {
  ...
  min-height: 0;
  border-radius: 14px;
  background: var(--td-bg-color-container, rgba(255,255,255,0.9));
...
.ws-studio-body {
  ...
  :deep(.artifact-list-view),
  :deep(.wiki-browser-container) {
    height: 100%;
  }
}
</style>
