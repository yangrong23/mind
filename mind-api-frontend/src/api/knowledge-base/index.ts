...
export function getKBCreationTemplate() {
  return get('/api/v1/knowledge-bases/creation-template');
}
...
// 从收件箱（__inbox__ KB）批量移动到目标知识库。
// 源由后端按调用方解析（每用户一个 inbox），最佳尝试：
// 单条失败不影响其他条目，逐条返回 failures。
export function moveFromInbox(data: {
  knowledge_ids: string[];
  target_kb_id: string;
}): Promise<{
  data: {
    ...
    moved: number;
    failed: number;
    failures?: { knowledge_id: string; error: string }[];
  };
}> {
  return post('/api/v1/inbox/move', data);
}
...
// 不指定知识库时，上传到调用方的默认收件箱（__inbox__ KB）。
// 后端 EnsureInbox 会按需懒创建。
export function uploadFileToInbox(
  data: { file: File; fileName?: string; metadata?: string; enable_multimodel?: boolean; channel?: string },
  ...
  return postUpload(`/api/v1/inbox/file`, formData, onProgress);
}
